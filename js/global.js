...
(function() {
    var presentations = qwery('.lanyrd-presentations')[0];
    // Are we loading Lanyrd presentations?
    if (presentations) {
        var h = qwery('head')[0],
            header = qwery('h3', presentations)[0],
            lanyrdTarget = qwery('.lanyrd-target-splat', presentations)[0],
            lanyrdLink = qwery('.lanyrd-splat', lanyrdTarget[0])[0],
            lanyrdScript,
            lanyrdList,
            count;
        function lanyrdInit() {
        ...
        function lanyrdCheck(upcoming) {
            ...
            lanyrdList = qwery('ol', lanyrdTarget)[0];
            if (lanyrdList) {
                clearInterval(lanyrdLoaded);
                var entries = qwery('li', lanyrdList);
                if (entries.length == 0) {
